...
class My_model_notices extends CI_Model {
    ...
    function get_latest_news($limit_) {
        ...
        $query = $this->db->get('notices', 0, $limit_);
        return $query->result();
    }
    ...
    function delete_news_events($id_) {
        ...
        $this->db->where('ID', $id_);
        $bool_ = $this->db->delete('notices');
        if ($bool_ == TRUE) {
            if ($file__ != 'x') {
                $full_path_ = FCPATH . '_assets_/notices/' . $file__;
                @unlink($full_path_);
            }
        }
        return $bool_;
